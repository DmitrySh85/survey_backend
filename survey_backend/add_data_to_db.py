...
def insert_df_to_db(df: pd.DataFrame) -> dict:
    for _, row in df.iterrows():
        ...
        text = row_values[0]
        first_answer = str(row_values[1]) if row_values[1] else ""
        second_answer = str(row_values[2]) if row_values[2] else ""
        third_answer = str(row_values[3]) if row_values[3] else ""
        fourth_answer = str(row_values[4]) if row_values[4] else ""
